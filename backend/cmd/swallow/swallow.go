...
func run(ctx context.Context) error {
	...
	logger = logger.Named("SwallowGo_app")
	defer func() { _ = logger.Sync() }()

	pool, err := pgxpool.New(ctx, fmt.Sprintf(
		"user=%s password=%s host=%s port=%s dbname=%s",
		os.Getenv("SWALLOWGO_DATABASE_USER"),
		os.Getenv("SWALLOWGO_DATABASE_PASSWORD"),
		os.Getenv("SWALLOWGO_DATABASE_HOST"),
		os.Getenv("SWALLOWGO_DATABASE_PORT"),
		os.Getenv("SWALLOWGO_DATABASE_NAME"),
	))
	if err != nil {
		return err
	}
